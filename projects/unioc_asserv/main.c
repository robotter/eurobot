...
// ROME interfaces
rome_reader_t rome_reader_strat;
rome_reader_t rome_reader_r3d2;
  ...
  switch(frame->mid) {
    ...
    case ROME_MID_ASSERV_ACTIVATE: {
      if(adxrs_get_calibration_mode()) {
        ROME_LOG(UART_STRAT, DEBUG, "ASSERV_ACTIVATE refuse, gyro is in calibration");
      }
      else {
    ...
    case ROME_MID_ASSERV_AUTOSET: {
      ...
      ROME_LOGF(UART_STRAT, DEBUG, "ASSERV AUTOSET %1.1f %1.1f %d %d", x,y,table_side,robot_side);
    ...
    case ROME_MID_ASSERV_RUN_TRAJ: {
      ...
      if(nxy % 2 != 0) {
        ROME_LOGF(UART_STRAT, ERROR, "asserv_run_traj order ignored: odd number of coordinates (%u)", nxy);
      } else {
        vect_xy_t xy[nxy/2];
        for(uint8_t i=0; i<nxy; i+=2) {
          xy[i/2].x = frame->asserv_run_traj.xy[i];
          xy[i/2].y = frame->asserv_run_traj.xy[i+1];
          ROME_LOGF(UART_STRAT, DEBUG, "run_traj[%i] %f %f", i, xy[i/2].x, xy[i/2].y);
        }
    ...
    case ROME_MID_ASSERV_SET_ZGYRO_SCALE: {
      INTLVL_DISABLE_ALL_BLOCK() {
        ...
        ROME_LOGF(UART_STRAT, DEBUG, "new gyro scale is %e", (double)zgyro_scale);
        rome_reply_ack(UART_STRAT, frame);
      }
    } break;
    case ROME_MID_ASSERV_GYRO_INTEGRATION: {
      INTLVL_DISABLE_ALL_BLOCK() {
        ...
        adxrs_integrate(b);
        ROME_LOGF(UART_STRAT, DEBUG, "gyro integration is %s", b ? "active":"inactive");
        rome_reply_ack(UART_STRAT, frame); 
      }
    } break;
    ...
    case ROME_MID_ASSERV_SET_SERVO: {
      INTLVL_DISABLE_ALL_BLOCK(){
        ...
        ROME_LOGF(UART_STRAT, DEBUG, "set servo %u to %u", frame->asserv_set_servo.id, frame->asserv_set_servo.value);
...
void rome_r3d2_handler(const rome_frame_t *frame) {
  // forward to strat
  rome_send(UART_STRAT, frame);
}
...
int main(void)
{
  ...
  PORTQ.OUT = 3;
  // Initialize ROME
  rome_reader_init(&rome_reader_strat, UART_STRAT);
  rome_reader_init(&rome_reader_r3d2, UART_R3D2);
...
#endif
    {
      ROME_LOGF(UART_STRAT, DEBUG, "gyro cal done ! off=%d sqsd=%f", offset, offset_sqsd);
      break;
    }

    // downlink debug infos
    ROME_LOGF(UART_STRAT, DEBUG, "gyro cal off=%d sqsd=%f", offset, offset_sqsd);
    rome_reader_handle_input(&rome_reader_strat, rome_strat_handler);
    _delay_ms(500);
  }
  ...
  for(;;) {
    ...
    _delay_ms(10);
    rome_reader_handle_input(&rome_reader_strat, rome_strat_handler);
    rome_reader_handle_input(&rome_reader_r3d2, rome_r3d2_handler);
  }
}
