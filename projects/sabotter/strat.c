...
// Check if opponent is close (R3D2_STOP_DISTANCE)
// If opponent is close, disable asserv and wait until time_end for opponent to
// move away.
detection_path_t opponent_detected_close(float angle, uint32_t time_end) {
...
// Check if opponent is on the way (R3D2_AVOID_DISTANCE)
// If opponent is close, move slightly backwards and wait at most
// DEFAULT_DETECTION_WAIT_MS for it to move away.
// Disable asserv if opponent gets too close (see opponent_detected_close()).
detection_path_t opponent_detected_carrot(int16_t x, int16_t y) {
  ...
  for(;;){
    ...
    if(opponent_detected_close(angle, time_end) == DETECTION_PATH_BLOCKED) {
...
order_result_t goto_pathfinding_node(uint8_t goal, float angle){
  for(;;){
    ...
    for(;;) {
      ...
      if(opponent_detected_close(angle, time + DEFAULT_DETECTION_WAIT_MS * 1000UL) == DETECTION_PATH_BLOCKED) {
        // timeout on close opponent
        return ORDER_DETECTION;
      }
...
#define PATHFINDING_GRAPH_NODE_WATER_DISPENSER_NEAR  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_WATER_DISPENSER_NEAR, PATHFINDING_GRAPH_NODE_ORANGE_WATER_DISPENSER_NEAR)
#define PATHFINDING_GRAPH_NODE_WATER_DISPENSER_FAR  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_WATER_DISPENSER_FAR, PATHFINDING_GRAPH_NODE_ORANGE_WATER_DISPENSER_FAR)
#define PATHFINDING_GRAPH_NODE_WATER_RECYCLE  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_WATER_RECYCLE, PATHFINDING_GRAPH_NODE_ORANGE_WATER_RECYCLE)
#define PATHFINDING_GRAPH_NODE_WATER_TOWER  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_WATER_TOWER, PATHFINDING_GRAPH_NODE_ORANGE_WATER_TOWER)
#define PATHFINDING_GRAPH_NODE_BEE  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_BEE, PATHFINDING_GRAPH_NODE_ORANGE_BEE)
#define PATHFINDING_GRAPH_NODE_OPPOSITE_BEE  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_ORANGE_BEE, PATHFINDING_GRAPH_NODE_GREEN_BEE)
#define PATHFINDING_GRAPH_NODE_CONSTRUCTION_AREA  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_TOP, PATHFINDING_GRAPH_NODE_ORANGE_TOP)
#define PATHFINDING_GRAPH_NODE_CONSTRUCTION_AREA_CUBES  TEAM_SIDE_VALUE(PATHFINDING_GRAPH_NODE_GREEN_MIDH, PATHFINDING_GRAPH_NODE_ORANGE_MIDH)
