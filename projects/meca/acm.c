...
void acm_update_arm_position(acm_t *s)
{
  ...
  s->cake_pos_mm = cake_pos_mm;

  double cake_perimeter_mm = s->cake_radius * M_PI;

  double second_lvl_red_robot_stall_blue_side_blow_last_candle_threshold_mm = cake_perimeter_mm - s->second_lvl_red_robot_stall_blue_side_blow_last_candle_offset_mm;
...
//printf("lid %u\trid %u\n", left_arm_candle_id, right_arm_candle_id);
  ...
  // avoid last blue candle (candle n°2 because 
  else if (second_lvl_arm_candle_id >= (SECOND_LVL_CANDLE_NB-2) && s->robot_color == ACM_RED && s->cake_stall_side == ACM_RED)
  {
    acm_move_arm(s, ACM_ARM_SECOND_LVL, ACM_ARM_STALLING);
  }
 //blow last red candle (not accessible when robot blocked to cake border)
  else if (cake_pos_mm>= second_lvl_red_robot_stall_blue_side_blow_last_candle_threshold_mm  && s->robot_color == ACM_RED && s->cake_stall_side == ACM_BLUE)
  //second_lvl_arm_candle_id >= SECOND_LVL_CANDLE_NB-1  && s->robot_color == ACM_RED && s->cake_stall_side == ACM_BLUE)
  {
...
void acm_init(acm_t *s)
{
  ...
  s->second_lvl_red_robot_stall_blue_side_blow_last_candle_offset_mm = DEFAULT_SECOND_LVL_RED_ROBOT_STALL_BLUE_SIDE_BLOW_LAST_CANDLE_OFFSET_MM; 
  ...
  s->cam_update_candle_window_mm = DEFAULT_ACM_CAM_UPDATE_CANDLE_WINDOW_MM;
