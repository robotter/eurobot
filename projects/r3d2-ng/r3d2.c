...
ISR(R3D2_SENSOR_INT_VECT) {
  ...
  bool pin = portpin_in(&R3D2_SENSOR_INT_PP);

  if(!pin) {
    portpin_outset(&LED_WEST_PP);
    r3d2.measure.start_tick_us = cnt;
  }
  else {
    portpin_outclr(&LED_WEST_PP);
    r3d2.measure.end_tick_us = cnt;
    _new_measure(&r3d2.measure);
    r3d2.measure.count++;
  }
}
...
void r3d2_init() {
  ...
  portpin_enable_int(&R3D2_SENSOR_INT_PP, R3D2_SENSOR_INT_NUM, INTLVL_LO);
}
